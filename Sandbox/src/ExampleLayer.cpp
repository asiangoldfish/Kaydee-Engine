  ...
  , cameraController(1280.0f / 720.0f, true)
{
    ...
    indexBuffer =
      Kaydee::IndexBuffer::create(indices, sizeof(indices) / sizeof(uint32_t));
...
void
ExampleLayer::onUpdate(Kaydee::Timestep ts)
{
    // KD_TRACE(
    ...
    Kaydee::Renderer::beginScene(cameraController.getCamera());
    {
        ...
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                ...
                Kaydee::Renderer::submit(flatColorShader, squareVA, transform);
            }
        }
        ...
        Kaydee::Renderer::submit(textureShader,
                                 squareVA,
...
void
ExampleLayer::onEvent(Kaydee::Event& e)
{
